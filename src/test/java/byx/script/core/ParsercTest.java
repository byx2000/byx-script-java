...
public class ParsercTest {
    ...
    private <R> void verifyParseResult(ParseResult<R> r, R expectedResult, int expectedBeforeIndex, int expectedRemainIndex) {
        assertEquals(expectedResult, r.result());
        assertEquals(expectedBeforeIndex, r.before().index());
        assertEquals(expectedRemainIndex, r.remain().index());
    }
    ...
    public void testSeq3() {
        ...
        assertEquals(Collections.emptyList(), r2.result());
        assertEquals(0, r2.before().index());
        assertEquals(0, r2.remain().index());
    }
    ...
    public void testMapException1() {
        Parser<String> p = str("hello").mapException((cursor, throwable) -> {
            ...
            assertInstanceOf(ParseException.class, throwable);
            return new MyException(cursor, "byx");
        });
    ...
    public void testThen() {
        ...
        Parser<String> identifier = oneOf(alpha, underline)
                ...
                .map(p -> p.first() + p.second().stream().map(Objects::toString).collect(Collectors.joining()))
                .then(r -> keywords.contains(r.result())
                        ? fail(input -> new MyException(r.before(), "关键字不能作为标识符"))
                        : success(r.result()));
