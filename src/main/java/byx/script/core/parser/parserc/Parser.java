...
public interface Parser<R> {
    ...
    default R parse(String s) {
        return parse(new Cursor(s)).result();
    }
    ...
    default <R2> Parser<Pair<R, R2>> and(Parser<R2> rhs) {
        return cursor -> {
            ...
            ParseResult<R2> r2 = rhs.parse(r1.remain());
            return new ParseResult<>(new Pair<>(r1.result(), r2.result()), cursor, r2.remain());
        };
    }
    ...
    default <R2> Parser<R2> map(Function<R, R2> mapper) {
        return cursor -> {
            ...
            return new ParseResult<>(mapper.apply(r.result()), cursor, r.remain());
        };
    }
    ...
    default <R2> Parser<R2> then(Function<ParseResult<R>, Parser<R2>> flatMap) {
        return cursor -> {
            ParseResult<R> r = this.parse(cursor);
            return flatMap.apply(r).parse(r.remain());
        };
    }
